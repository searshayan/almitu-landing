...
- "checklist": { "intro": "warm reinforcement line", "style": "check|numbered", "items": [ { "text": "an 'I can ...' Can-Do statement OR a short tutor-facing recap point — follow exactly what this slide's brief asks for", "hint": "small hint or empty" } ], "footer": "the Next Step: name the post-session activity + note the tutor tracks completion time and metrics" }
...
- "applyreview":{ "application": { "instruction": "...", "bank": ["target items = answer pool"], "prompts": ["... ___ ..."], "answers": [ { "answer": "item for prompt 1", "feedback": "short feedback phrase" } ], "notes": "tutor-aid or empty" }, "review": { "can_do": "a Can-Do statement OR a short tutor-facing recap sentence — follow exactly what this slide's brief asks for", "next_step": "post-session activity + tutor tracks metrics" } }
...
const VOCAB_SKELETON_25 = [
  ...
  { icon: '', label: 'Review & Next Step', layout: 'checklist', brief: 'style:check. intro = one warm, specific tutor reinforcement line. items = EXACTLY 3 short, plain recap points a TUTOR can read aloud to quickly summarize what THIS session covered (the vocabulary theme, how many/which kinds of words, and the skill(s) practiced) — a factual recap, NOT "I can…" Can-Do statements. footer = the Next Step: name the level-appropriate post-session activity the student completes on the platform and note the tutor tracks completion time and practice metrics.' }
];
...
const GRAMMAR_SKELETON_25 = [
  ...
  { icon: '', label: 'Review & Next Step', layout: 'checklist', brief: 'style:check. intro = one warm, specific tutor reinforcement line. items = EXACTLY 3 short, plain recap points a TUTOR can read aloud to quickly summarize what THIS session covered (the grammar point taught, its key form/use, and the practice done) — a factual recap, NOT "I can…" Can-Do statements. footer = the Next Step: name the level-appropriate post-session activity and note the tutor tracks completion time and practice metrics.' }
];
...
const GRAMMAR_SKELETON_15 = [
  ...
  { icon: '', label: 'Review & Next Step', layout: 'checklist', brief: 'style:check. intro = one warm reinforcement line. items = EXACTLY 3 short, plain recap points a TUTOR can read aloud to summarize what this session covered (the grammar point and the practice done) — a factual recap, NOT Can-Do statements. footer = the Next Step: post-session activity + tutor tracks completion time and metrics.' }
];
...
const COMM_SKELETON_25 = [
  ...
  { icon: '', label: 'Review & Next Step', layout: 'checklist', brief: 'style:check. intro = one warm, specific tutor reinforcement line. items = EXACTLY 3 short, plain recap points a TUTOR can read aloud to quickly summarize what THIS session covered (the scenario/situation, the key expressions taught, and the speaking practice done) — a factual recap, NOT "I can…" Can-Do statements. footer = the Next Step: name the level-appropriate post-session activity and note the tutor tracks completion time and practice metrics.' }
];
...
- Do NOT write real URLs, video titles, channels or publishers — you cannot verify them and they would be dead links. Instead write precise SEARCH QUERIES; the app turns them into live YouTube / TED / article search links that always work.
- videoQuery: a specific, natural query that finds a genuinely helpful video for THIS topic at THIS level (e.g. everyday conversation, listening practice, or a clear explanation). Level-appropriate wording.
- tedQuery: ONLY for B2/C1/C2 — a query for a TED Talk or similar richer talk on the broader theme; for lower levels leave it "".
- articleQuery: a query that finds a simple article or explanation tied to this session's grammar / vocabulary theme / communication focus / objective.
- videoLabel and articleLabel: a short, human label of what each resource is about (shown on the link).
- intro: one concise invitation tied to the session goal.
